  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav className={`absolute inset-x-0 top-full bg-white/95 pb-6 shadow-lg transition-all duration-300 dark:bg-slate-950/95 ${open ? 'visible opacity-100' : 'invisible opacity-0'} sm:static sm:shadow-none sm:bg-transparent sm:pb-0 sm:opacity-100 sm:visible`}>
            <div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 pt-4 sm:flex-row sm:items-center sm:gap-6 sm:pt-0">
              <Link to="/courses" className="text-sm font-medium text-slate-700 hover:text-brand-600 dark:text-slate-200">Cours</Link>
              <Link to="/dashboard" className="text-sm font-medium text-slate-700 hover:text-brand-600 dark:text-slate-200">Mon espace</Link>
              <Link to="/admin" className="text-sm font-medium text-slate-700 hover:text-brand-600 dark:text-slate-200">Administration</Link>
              <div className="mt-3 flex gap-3 sm:mt-0">
                <Link to="/login" className="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-brand-300 hover:text-brand-700 dark:border-slate-700 dark:text-slate-200">Connexion</Link>
                <Link to="/register" className="rounded-full bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-700">Inscription</Link>
              </div>
            </div>
          </nav>
        </div>
      ...
      <footer className="border-t border-slate-200 bg-white py-8 text-center text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400">
        LevelUp © 2026 · Plateforme e-learning fictive
      </footer>
    </div>
  );
}
