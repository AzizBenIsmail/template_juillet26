...
const benefitKeys = ['modernCourses', 'trackedProgress', 'community'];
const benefitIcons = [BookOpen, ShieldCheck, Star];
    ...
    <div className="space-y-16">
      <section className="rounded-[2rem] bg-hero-gradient px-6 py-16 text-white shadow-soft sm:px-14">
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              <p className="mb-4 text-sm uppercase tracking-[0.3em] text-slate-200/90">{t('hero.tagline')}</p>
              <h1 className="text-4xl font-semibold sm:text-5xl">{t('hero.title')}</h1>
              <p className="mt-6 max-w-2xl text-base text-slate-100/90">{t('hero.description')}</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {benefitKeys.map((key, index) => {
                ...
                return (
                  <Card key={key} className="bg-white/10 text-white shadow-none ring-1 ring-white/10">
                    <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/15 text-white">
                      <Icon className="h-6 w-6" />
                    </div>
                    <h3 className="mt-4 text-lg font-semibold">{t(`landing.benefits.${key}.title`)}</h3>
                    <p className="mt-3 text-sm text-slate-100/85">{t(`landing.benefits.${key}.description`)}</p>
                  </Card>
                );
              })}
            </div>
      ...
      <section className="grid gap-8 lg:grid-cols-3">
        <div className="space-y-5 rounded-3xl bg-white p-8 shadow-soft dark:bg-slate-950">
          <h2 className="text-2xl font-semibold">{t('landing.categories')}</h2>
          <div className="grid gap-3">
            <Badge>{t('categories.category-1')}</Badge>
            <Badge>{t('categories.category-2')}</Badge>
            <Badge>{t('categories.category-3')}</Badge>
            <Badge>{t('categories.category-4')}</Badge>
          </div>
        </div>
        <div className="space-y-5 rounded-3xl bg-white p-8 shadow-soft dark:bg-slate-950">
          <h2 className="text-2xl font-semibold">{t('landing.testimonials')}</h2>
        ...
        <div className="space-y-5 rounded-3xl bg-white p-8 shadow-soft dark:bg-slate-950">
          <h2 className="text-2xl font-semibold">{t('landing.recentCourses')}</h2>
          <div className="space-y-4">
